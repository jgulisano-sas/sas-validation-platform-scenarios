...
class st_analystoptimizeflow(PlaywrightUser):
    ...
    @task
    @pw
    async def test_st_analystoptimizeflow(self, page: PageWithRetry):
        ...
        async with event(self, "01: Starting New SAS st_analystoptimizeflow tests"):
          try:  
        ...
        async with event(self,"04: Run Data Flow"):
          try:
            ...
            time.sleep(3)
            await expect(page.get_by_test_id("tab-2").get_by_label("Processing").locator("svg")).not_to_be_visible(timeout=400000)
