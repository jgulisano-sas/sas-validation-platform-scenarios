...
class st_queryflow(PlaywrightUser):
    ...
    @task
    @pw
    async def test_st_queryflow(self, page: PageWithRetry):
        ...
        async with event(self, "01: Starting New SAS st_queryFlow tests"):
          try:  
        ...
        async with event(self,"04: Run Data Flow"):
          try:
            ...
            self.logger.info(f"Finished opening queryflow.flw program")
            self.logger.info(f"Data Flow: queryflow.flw successfully opened")
            await page.get_by_test_id("tab-2").get_by_text("queryFlow.flw").click()
            ...
            self.logger.info(f"Finished Runing queryFlow Data Flow")
          except:
            await exception_handling(f"Failed to run queryFlow Data Flow", user)
            raise
