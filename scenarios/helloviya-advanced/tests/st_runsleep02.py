...
class st_runsleep02(PlaywrightUser):
    ...
    @task
    @pw
    async def test_st_runsleep02(self, page: PageWithRetry):
        ...
        async with event(self, "01: Starting New studioProg02 tests"):
          try:  
        ...
        async with event(self,"04: Run SAS Program"):
            ...
            self.logger.info(f"Opening sleep_40.sas program")
            ...
            await page.get_by_test_id("member-list-grid-wrapper").get_by_text("sleep_40.sas").click()
            await page.get_by_test_id("file-folder-dialog-firstButton").click()
            self.logger.info(f"Finished opening sleep_40.sas program")
            await page.get_by_test_id("tab-group-bar-_root_").get_by_text("sleep_40.sas").click()
